...
def do_work_son( sProject, sUniqSelectorId, sUniqTargetId, sDomain, sIp, sProtocol, sOpenTcpPort, sTcpService ):
    ...
    sIp = sIp.strip()
    try:
        sSourceTool = 'traceroute_icmp'
        sBinaryPath = "/usr/sbin/traceroute"
        sCmdToExecute = sBinaryPath + ' -I ' + sIp
        #
        ...
        logging.debug("traceroute_icmp() sCmdOut: " + str(sCmdOut))
        #
        sSeverity = "INFO"
        sStringToFind = 'traceroute to'
        lCmdOut = sCmdOut.splitlines()
        for sLine in lCmdOut:
            logging.debug("traceroute_icmp() sLine: " + str(sLine))
            if sLine.find(sStringToFind) >= 0:
                logging.debug("traceroute_icmp() Skip")
            else:
                sLine = sLine.strip()
        ...
        logging.error("traceroute_icmp() subprocess.Popen failed :\ ")
...
class TracerouteIcmp:
    """ Event listening service. """
    name = "traceroute_icmp"
    ...
    def handle_event(self, payload):
        ...
        print("traceroute_icmp received:", payload)
