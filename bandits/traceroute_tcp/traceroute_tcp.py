...
def do_work_son( sProject, sUniqSelectorId, sUniqTargetId, sDomain, sIp, sProtocol, sOpenTcpPort, sTcpService ):
    ...
    sIp = sIp.strip()
    try:
        sSourceTool = 'traceroute_tcp'
        sBinaryPath = "/usr/sbin/traceroute"
        sCmdToExecute = sBinaryPath + ' -T -p 80 ' + sIp
        #
        ...
        logging.debug("traceroute_tcp() sCmdOut: " + str(sCmdOut))
        #
        sSeverity = "INFO"
        sStringToFind = 'traceroute to'
        lCmdOut = sCmdOut.splitlines()
        for sLine in lCmdOut:
            logging.debug("traceroute_tcp() sLine: " + str(sLine))
            if sLine.find(sStringToFind) >= 0:
                logging.debug("traceroute_tcp() Skip")
            else:
                sLine = sLine.strip()
        ...
        logging.error("traceroute_tcp() subprocess.Popen failed :\ ")
...
class TracerouteTcp:
    ...
    def handle_event(self, payload):
        ...
        print("traceroute_tcp received:", payload)
